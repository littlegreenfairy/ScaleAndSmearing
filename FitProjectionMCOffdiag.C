...
void FitProjectionMCOffdiag() {
    ...
    for (int i = 0; i < Nbins; i++) {
        ...
        TH1D *hist = (TH1D*)file->Get(Form("proj_offdiag_bin_%d", i + 1)); //qui per farlo con la massa regressed
        if (!hist) {
            std::cerr << "Istogramma non trovato: proj_offdiag_bin_" << i + 1 << std::endl;
            continue;
        }
        ...
        // Salva la canvas 
        canvas->SaveAs(Form("PlotConID2022/Butokjpsi/FitMC_Offdiag_id2022/fit_proj_bin_%d.png", i+1));
        // deallocazioni
        delete canvas;
        delete frame;
    ...
    // Definisci la variabile di massa invariante
        ...
        frame->Draw();
        WriteSimulation();
        canvas_incl->SaveAs("PlotConID2022/Butokjpsi/FitMC_Offdiag_id2022/fit_inclusivePt.png");
        delete canvas_incl;
    ...
    ////////////////////////// Salvo i parametri per inizializzare il fit dei dati
        // Crea un file ROOT
        TFile *file_param = TFile::Open("fit_results_offdiag.root", "RECREATE");

        // Crea un albero ROOT per memorizzare i dati
        TTree *tree = new TTree("fitResults", "Fit Results Off-diagonal");
